...
/**
 * From server to client after command, if no error and result set -- that is, if the command was a query which returned
 * a result set. The Result Set Header Packet is the first of several, possibly many, packets that the server sends for
 * result sets. The order of packets for a result set is:
 *
 * <pre>
